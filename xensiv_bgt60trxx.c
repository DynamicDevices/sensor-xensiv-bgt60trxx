...
static xensiv_bgt60trxx_device_t detect_device_type(uint32_t chipid)
{
    ...
    uint32_t chip_id_rf =
        (chipid & XENSIV_BGT60TRXX_REG_CHIP_ID_RF_ID_MSK) >> XENSIV_BGT60TRXX_REG_CHIP_ID_RF_ID_POS;

    if ((chip_id_digital == 3U) && (chip_id_rf == 3U)) {
        return XENSIV_DEVICE_BGT60TR13C;
    } else if ((chip_id_digital == 6U) && ((chip_id_rf == 6U) || (chip_id_rf == 11U))) {
...
int32_t xensiv_bgt60trxx_config(xensiv_bgt60trxx_t *dev, const uint32_t *regs, uint32_t len)
{
    ...
    int32_t status = xensiv_bgt60trxx_soft_reset(dev, XENSIV_BGT60TRXX_RESET_SW);
    ...
    if (XENSIV_BGT60TRXX_STATUS_OK == status) {
        ...
        for (uint32_t reg_idx = 0; reg_idx < len; ++reg_idx) {
            ...
            uint32_t reg_addr =
                ((val & XENSIV_BGT60TRXX_SPI_REGADR_MSK) >> XENSIV_BGT60TRXX_SPI_REGADR_POS);
            uint32_t reg_data =
                ((val & XENSIV_BGT60TRXX_SPI_DATA_MSK) >> XENSIV_BGT60TRXX_SPI_DATA_POS);
...
int32_t xensiv_bgt60trxx_set_reg(const xensiv_bgt60trxx_t *dev, uint32_t reg_addr, uint32_t data)
{
    ...
    int32_t status = xensiv_bgt60trxx_platform_spi_transfer(
        dev->iface, (uint8_t *) &temp, NULL, XENSIV_BGT60TRXX_SPI_REG_XFER_LEN_BYTES);
...
int32_t xensiv_bgt60trxx_get_reg(const xensiv_bgt60trxx_t *dev, uint32_t reg_addr, uint32_t *data)
{
    ...
    xensiv_bgt60trxx_platform_spi_cs_set(dev->iface, 0);
    int32_t status = xensiv_bgt60trxx_platform_spi_transfer(
        dev->iface, (uint8_t *) &temp, (uint8_t *) data, XENSIV_BGT60TRXX_SPI_REG_XFER_LEN_BYTES);
    xensiv_bgt60trxx_platform_spi_cs_set(dev->iface, 1);
                                       ...
                                       uint32_t num_samples)
{
    ...
    int32_t retval =
        xensiv_bgt60trxx_platform_spi_transfer(dev->iface,
                                               (uint8_t *) &reg_addr,
                                               (uint8_t *) &gsr0,
    ...
    if (XENSIV_BGT60TRXX_STATUS_OK == retval) {
        if ((gsr0 &
             (XENSIV_BGT60TRXX_REG_GSR0_FOU_ERR_MSK | XENSIV_BGT60TRXX_REG_GSR0_SPI_BURST_ERR_MSK |
              XENSIV_BGT60TRXX_REG_GSR0_CLK_NUM_ERR_MSK)) == 0U) {
            retval = xensiv_bgt60trxx_platform_spi_fifo_read(dev->iface, data, num_samples);
        } else {
...
int32_t xensiv_bgt60trxx_start_frame(const xensiv_bgt60trxx_t *dev, bool start)
{
    ...
    if (start) {
        ...
        status = xensiv_bgt60trxx_soft_reset(dev, XENSIV_BGT60TRXX_RESET_FSM);
    }
...
{
    uint16_t next_value = (cur_test_word >> 1) | (((cur_test_word << 11) ^ (cur_test_word << 10) ^
                                                   (cur_test_word << 9) ^ (cur_test_word << 3)) &
                                                  0x0800U);
