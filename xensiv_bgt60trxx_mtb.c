 ...
 *******************************************************************************/
     ...
     * Macros
     *******************************************************************************/
    #define XENSIV_BGT60TRXX_ERROR(x)                                                              \
        (((x) == XENSIV_BGT60TRXX_STATUS_OK)                                                       \
             ? CY_RSLT_SUCCESS                                                                     \
                                    ...
                                    size_t len)
{
    ...
    cy_rslt_t rslt = cyhal_gpio_init(selpin, CYHAL_GPIO_DIR_OUTPUT, CYHAL_GPIO_DRIVE_STRONG, true);

    if (CY_RSLT_SUCCESS == rslt) {
        rslt = cyhal_gpio_init(rstpin, CYHAL_GPIO_DIR_OUTPUT, CYHAL_GPIO_DRIVE_STRONG, true);
    }
...
__STATIC_INLINE void spi_set_data_width(CySCB_Type *base, uint32_t data_width)
{
    ...
    CY_REG32_CLR_SET(SCB_TX_CTRL(base), SCB_TX_CTRL_DATA_WIDTH, (uint32_t) data_width - 1U);
    CY_REG32_CLR_SET(SCB_RX_CTRL(base), SCB_RX_CTRL_DATA_WIDTH, (uint32_t) data_width - 1U);
}
                                               ...
                                               uint32_t len)
{
    ...
    cy_en_scb_spi_status_t status = Cy_SCB_SPI_Transfer(
        mtb_iface->spi->base, tx_data, rx_data, len, &(mtb_iface->spi->context));
    ...
    return ((CY_SCB_SPI_SUCCESS == status) ? XENSIV_BGT60TRXX_STATUS_OK
...
int32_t xensiv_bgt60trxx_platform_spi_fifo_read(void *iface, uint16_t *rx_data, uint32_t len)
{
    ...
    cy_en_scb_spi_status_t status =
        Cy_SCB_SPI_Transfer(mtb_iface->spi->base, NULL, rx_data, len, &(mtb_iface->spi->context));
    if (CY_SCB_SPI_SUCCESS == status) {
        while (0UL !=
               (CY_SCB_SPI_TRANSFER_ACTIVE &
                Cy_SCB_SPI_GetTransferStatus(mtb_iface->spi->base, &(mtb_iface->spi->context)))) {
                            ...
                            void *callback_arg)
{
    ...
    if (NULL == callback) {
        ...
        if (init) {
            result = cyhal_gpio_init(pin, CYHAL_GPIO_DIR_INPUT, CYHAL_GPIO_DRIVE_PULLDOWN, false);
        }
